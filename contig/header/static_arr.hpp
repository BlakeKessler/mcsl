...
template <typename T, uint _size> class [[clang::trivial_abi]] mcsl::static_arr : public contig_base<T> {
   ...
   public:
      ...
      constexpr static_arr(const contig_t auto& other) requires requires{ other.size() >= _size; } { assert(other.size() >= _size, __OVERSIZED_COPY_MSG); for (uint i = 0; i < _size; ++i) { _buf[i] = other[i]; }}
