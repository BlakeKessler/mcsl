...
template<typename T> struct mcsl::contig_base {
   ...
   [[gnu::pure]] constexpr explicit operator bool(this auto&& obj) { return obj.size() && obj.data(); }
   ...
   [[gnu::pure]] constexpr iterator_t<T> auto begin(this auto&& obj)   { return obj.begin(); }
   ...
   [[gnu::pure]] inline constexpr auto at(this auto&& obj, const uint i) -> decltype(auto) { assert(i < obj.size()); return *(obj + i); }
